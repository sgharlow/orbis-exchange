...
- A **64×64 grid of resource cells** evolves every 3 seconds by Conway-style cellular-automaton rules. Regions bloom, spread, overcrowd, and collapse. Scarcity is *emergent* — nothing is hand-authored. The field renders as a crisp single-hue density heatmap (brightness = abundance; scroll to zoom in, then scroll or drag to pan), and you can light up one commodity at a time with reveal-layer chips.
...
- **Algorithmic agents are first-class players.** They trade through the exact same order path a human does, at zero inference cost. Market-makers and a per-commodity liquidity *pulse* keep the book two-sided and the tape warm even in a near-empty room; momentum, value and a cross-commodity arbitrage bot are the opponents you race on one net-worth leaderboard.
