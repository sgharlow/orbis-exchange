...
### 3c. Scout runaway — ✅ FIXED + DEPLOYED 2026-06-20 (Design B / Tier 1)
The `scout-r0` net-worth runaway (claim-only → quadratic, 46× the field / billions over hours) is fixed: the scout is now a **bounded supplier** (caps at 5 cells, sells its mined output passively). Worker Lambda redeployed (SHA `7oAsugyr…`) + live world re-seeded. **Live-verified** (gen 79 soak): top/median ratio **1.01×**, scout rank **14/14**, cells 5, 127 open sells, prices healthy ~100. **No more reset-before-record needed** for normal sessions. Root-cause + the deeper "consumption sink" fix (Tier 2) are in [`roadmap-economy-tier2.md`](roadmap-economy-tier2.md). Code: `6cb2bff`; TDD test added.
