...
| `GET /api/leaderboard` | ✅ 200 | the **9 strategic opponents** (momentum/value/arb) on the ~1.5M baseline; makers/pulse/scout are off-board infra (`kind='market'`), which closes the old scout runaway — see note 3 |
| `/world` UI | ✅ renders | living-world canvas rendered as a **crisp single-hue (cyan) density heatmap** (brightness = abundance; scroll to zoom, scroll/drag to pan) + 3 panels (LIVING WORLD / GLOBAL MARKET / LEADERBOARD AI VS HUMAN); on-demand reveal-layer chips, "my cells" spotlight, inline cell tooltip; an always-visible objective rail (Enter → Claim → Sell) + first-visit how-to card. Opening the link **auto-joins you as a guest — no login/signup/handle prompt**; rename inline anytime. |
| Full test suite `pnpm -r test` | ✅ **143 green** | db 55 · web 36 · worker 52 |
...
3. ✅ **RESOLVED — clean re-seed (latest 2026-06-24).** The bot economy is an **18-bot roster**: a per-commodity liquidity **pulse** + market-makers + a mining **scout** are `kind='market'` and run the book **off the leaderboard**, so the board shows the **9 strategic opponents** (momentum / value / arb) at the ~1.5M baseline. This off-board reclassification is what kills the old scout net-worth runaway (the 12-cell cap alone did **not** hold long-run). The world is public and keeps accumulating guests, so **do re-seed right before recording** (`pnpm db:reseed-live`, ~90s) and **join as your demo player after** it finishes. Settlement is live-verified (join → market Buy → filled, balance debited, trade on tape). Demo-ready.
...
| Gate | State | Evidence |
|---|---|---|
| Full test suite | ✅ 143 green | `pnpm -r test` (db 55 · web 36 · worker 52) |
| Live health + read path | ✅ | §A above |
| Market liquid (books populated, agents trading) | ✅ | §A — books populated; a per-commodity liquidity **pulse** keeps both sides quoted and the tape warm even in a sparse room. The old `scout-r0` runaway is closed by moving makers/pulse/scout off the board (`kind='market'`); re-seed right before recording for a pristine board (see [SUBMISSION-STATUS §3b](SUBMISSION-STATUS.md)) |
...
1b. **Re-seed the demo world right before recording** — the live site keeps accumulating guests, so run `pnpm db:reseed-live` (~90s; pauses worker → resets to the ~1.5M baseline → resumes) and **join as your demo player after** it finishes. Confirm `/api/leaderboard` returns the **9 strategic opponents** (momentum / value / arb) and **0** `mm-*`/`pulse-*`/`scout` (those are `kind='market'`, intentionally off the board). *(~5 min)*
