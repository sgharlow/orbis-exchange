...
3. ✅ **PARTLY RESOLVED 2026-06-19 — leaderboard is now a clean 14 agents, 0 humans.** The 2 leftover dev fixtures (`alice`/`bot-maker`) were deleted from live DSQL (orphan-checked first). ⚠️ **Remaining demo-quality call:** this was a *targeted* delete, so the 14 agents kept their **old accumulated credits** (~1.5M; **`scout-r0` is a runaway ~5M+ and climbing**). The settlement mechanic is live-verified (join→crossing buy→filled@102, balance debited, trade on tape). For a more compelling, near-parity leaderboard on camera, consider a **full fresh re-seed** (wipe + re-seed to gen 0 / 1.0M) — submittable as-is either way.
